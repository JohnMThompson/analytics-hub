      ...
      <thead>
        <tr className="border-b" style={{ borderColor: 'var(--border-soft)' }}>
          {columns.map((column) => (
            <th
              key={column.key}
              className={`py-3 px-4 font-semibold ${column.align === 'right' ? 'text-right' : 'text-left'}`}
      ...
      <tbody>
        {rows.map((row, idx) => (
          <tr
            ...
            className={striped && idx % 2 === 0 ? 'bg-slate-50' : ''}
          >
            {columns.map((column) => (
              <td
                ...
                style={{ color: column.tone === 'primary' ? 'var(--text-primary)' : 'var(--text-secondary)' }}
              >
