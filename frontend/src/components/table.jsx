...
export function DataTable({
  ...
  const handleExportConfirm = () => {
    ...
    if (format === 'csv') {
      downloadTableAsCsv({ columns, rows, fileName: exportFileName });
    } else {
      downloadTableAsExcel({ columns, rows, fileName: exportFileName, sheetName: exportSheetName });
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {showExportActions && (
        <div className="flex flex-wrap items-center justify-end border-t px-4 pt-4 pb-4" style={{ borderColor: 'var(--border-soft)' }}>
          <button
            type="button"
            className="focus-ring inline-flex items-center rounded-full border px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em]"
      ...
      {showExportActions && isExportDialogOpen && (
